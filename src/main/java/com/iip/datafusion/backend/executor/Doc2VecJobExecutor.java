...
public class Doc2VecJobExecutor extends AbstractTerminatableThread implements JobExecutor<Doc2VecJob> {
    ...
    protected void doRun() throws Exception {
        ...
        JobRegistry.getInstance().update(doc2vecJob, JobStatusType.EXECUTING);//
        try{
            doJob(doc2vecJob);
            JobRegistry.getInstance().update(doc2vecJob, JobStatusType.SUCCESS);//
        } catch (Exception e){
            e.printStackTrace();
            JobRegistry.getInstance().update(doc2vecJob, JobStatusType.ERROR);
        } finally {
